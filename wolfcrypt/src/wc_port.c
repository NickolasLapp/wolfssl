...
int wolfCrypt_Init()
{
    ...
    void CustomLock(int type, int mode)
    {
        if(dyn_locking_cb)
            dyn_locking_cb(
        else if(locking_cb)
           locking_cb(mode, type, void,0);
    }

    void CustomDynLock(wolfssl_dynamic_locking_cb lock)
    {
        dyn_locking_cb = lock;
    }

    void CustomDynCreate(wolfssl_dynamic_create_cb create)
    {
        dyn_create_cb = create;
    }

    void CustomDynDestroy(wolfssl_dynamic_free_cb free)
    {
        dyn_free_cb = free;
    }
